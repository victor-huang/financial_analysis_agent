...
def find_latest_reported_quarter(eps_data):
    ...
    for eps in eps_data:
        ...
        if eps_actual is None or (isinstance(eps_actual, float) and str(eps_actual) == 'nan'):
            continue
...
def find_latest_reported_revenue(revenue_data):
    ...
    for rev in revenue_data:
        ...
        # Skip if no actual data or if it's NaN
        if revenue_actual is None or (isinstance(revenue_actual, float) and str(revenue_actual) == 'nan'):
            continue
...
def generate_csv_row(json_data, target_quarter=None):
    ...
    market_segment = f"{sector}/{industry}" if sector and industry else sector or industry
    ...
    if target_quarter is None:
        ...
        if not target_quarter:
            print("Warning: Could not determine current quarter, using latest reported quarter")
    ...
    if quarter_eps:
        ...
        if eps_actual is not None and isinstance(eps_actual, float) and str(eps_actual) == 'nan':
            eps_actual = None

        row['EPS Q estimate'] = f"{eps_estimate:.2f}" if eps_estimate else ''
        row['EPS Q current actual'] = f"{eps_actual:.2f}" if eps_actual else ''
        ...
        if prior_year_eps and prior_year_eps.get('eps_actual'):
            ...
            row['EPS Q last year'] = f"{prior_eps:.2f}"

            # Calculate EPS YoY %
            # Use actual if available, otherwise use estimate for projection
            current_eps = eps_actual if eps_actual else eps_estimate
            if current_eps and prior_eps and prior_eps > 0:
                eps_yoy = calculate_yoy_percentage(current_eps, prior_eps)
                row['EPS YoY %'] = f"{eps_yoy:.2f}" if eps_yoy is not None else ''
    ...
    if quarter_revenue:
        ...
        if revenue_actual is not None and isinstance(revenue_actual, float) and str(revenue_actual) == 'nan':
            revenue_actual = None

        # Convert to billions
        row['Revenue Q estimate'] = f"{revenue_estimate/1_000_000_000:.2f}" if revenue_estimate else ''
        row['revenue Q actual'] = f"{revenue_actual/1_000_000_000:.2f}" if revenue_actual else ''
        ...
        if prior_year_revenue and prior_year_revenue.get('revenue_actual'):
            ...
            row['Revenue same Q last year'] = f"{prior_rev/1_000_000_000:.2f}"

            # Calculate Revenue YoY %
            # Use actual if available, otherwise use estimate for projection
            current_rev = revenue_actual if revenue_actual else revenue_estimate
            if current_rev and prior_rev and prior_rev > 0:
                rev_yoy = calculate_yoy_percentage(current_rev, prior_rev)
                row['Revenue actual YoY %'] = f"{rev_yoy:.2f}" if rev_yoy is not None else ''
        ...
        if prior_quarter_revenue and prior_quarter_revenue.get('revenue_actual'):
            ...
            if prior_q_year_revenue and prior_q_year_revenue.get('revenue_actual'):
                ...
                if prior_q_prior_year and prior_q_prior_year > 0:
                    last_q_yoy = calculate_yoy_percentage(prior_q_rev, prior_q_prior_year)
                    row['Revenue last Q YoY %'] = f"{last_q_yoy:.2f}" if last_q_yoy is not None else ''
...
def generate_csv(input_file, output_file=None, target_quarter=None):
    ...
    # Write CSV
    fieldnames = [
        'ticker', 'quarter', 'hot?', 'Note', 'Company name', 'Market segment', 'Market Cap (B)',
        'Fast grow?', 'HC change (%)', 'tech/analyst', 'post gain $', '2nd day gain %',
        'EPS Q estimate', 'EPS Q current actual', 'EPS beat %',
        'Revenue Q estimate', 'revenue Q actual', 'Revenue Q Beat %',
        'EPS Q last year', 'EPS YoY %',
        'Revenue same Q last year', 'Revenue actual YoY %', 'Revenue last Q YoY %'
    ]
    ...
    print(f"  EPS: {row['EPS Q current actual']} (estimate: {row['EPS Q estimate']})")
    print(f"  Revenue: {row['revenue Q actual']}B (estimate: {row['Revenue Q estimate']}B)")
...
Examples:
  ...
  python generate_analysis_csv.py aapl.json aapl_analysis.csv --quarter 2025Q2
    ...
    parser.add_argument('--quarter', '-q', help='Quarter to analyze (e.g., 2025Q3). Default: current quarter (latest unreported)')
