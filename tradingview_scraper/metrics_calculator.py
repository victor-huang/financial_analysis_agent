...
def calculate_yoy_percentage(
    current: Optional[float], last_year: Optional[float]
) -> Optional[float]:
    """
...
def format_revenue(revenue: Optional[float]) -> str:
    """
    Format revenue value in millions (no suffix, full precision).

    Args:
        revenue: Revenue value (in millions from scraper)

    Returns:
        Numeric string in millions or empty
    """
    if revenue is None:
        return ""

    # Keep in millions, full precision, no suffix
    return str(revenue)
